...
const Schedule = ({ route, navigation }) => {
    ...
    const createSchedule = async (id, date) => {
        ...
        user = JSON.parse(user); // Chuyển đổi user từ chuỗi JSON sang đối tượng JavaScript nếu cần

        // Lấy user.id ra để sử dụng
                            ...
                            {selectedScheduleId === schedule.id && (
                                <>
                                    ...
                                    <View style={styles.timeSlots}>
                                        {["7-8", "9-10", "10-11", "11-12", "12-13", "13-14", "15-16", "17-18", "19-20"].map((time) => (
                                            <TouchableOpacity
                                                key={time}
